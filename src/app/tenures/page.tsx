...
import { getTenures, Tenure } from "@/lib/archive";
...
export default async function ArchivePage() {
  ...
  const data = [
    {
      ...
      content: (
        <div>
          <h2 className="mb-4 text-2xl md:text-4xl font-semibold">The Tenure of His Manifest Glory</h2>
          <p className="mb-8 font-normal text-sm md:text-base text-muted-foreground">
            "Phaneros Doxa"
          </p>
          <div>
            <Image
              src="https://res.cloudinary.com/dpjo7lpww/image/upload/v1781395780/IMG_6732_qynayj.jpg"
              alt="banner"
              width={2160}
              height={2700}
  ...
  // Choose a uniform template for all pasts tenures
  // const data = tenures.map((tenure) => {
  //   return {
  //     title: tenure.year.split("/")[1],
  //     content: (
  //       <div>
  //         <h2 className="mb-4 text-2xl md:text-4xl font-semibold">{tenure.theme}</h2>
  //         <p className="mb-8 font-normal text-sm md:text-base text-muted-foreground">
  //           {tenure.description}
  //         </p>
  //         <div>
...
export function TenureCard({ tenure }: { tenure: Tenure }) {
  return (
    <div className="w-full flex flex-col gap-2 pb-6">
      <div className="w-full relative bg-muted aspect-video rounded-md" />
      <div className="">
        <h4 className="font-medium">{tenure.theme}</h4>
      </div>
    </div>
  )
}
